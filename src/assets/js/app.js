var wins = 0;
var gamePlay = false;
var guessesStart, guessesLeft, gamePlayEasy, keyLetter, keyCode;
var randomWordFromArray = [];
var blankWordArray = [];
var wrongLettersArray = [];
var wordSplitToArray = [];
var easyWords = ['border', 'cookies', 'plates', 'monkey', 'relationship', 'university', 'rabbit', 'hamburger'];
var hardWords = ['crypt', 'fjord', 'haiku', 'jinx', 'klutz', 'phlegm', 'sphinx', 'yacht'];
...
function init() {
    ...
    document.querySelector('#alreadyGuessedHeading').textContent = 'Letters Guessed';   
}
function reset() {
    wins = '-';
    guessesStart = '-';
    ...
    document.querySelector('#wordToGuessHeading').textContent = '';    
    document.querySelector('#wordToGuess').textContent = 'Choose a difficulty level';    
    document.querySelector('#alreadyGuessedHeading').textContent = 'Then start guessing!';
...
function selectNewWord() {
    if (gamePlayEasy) {
        randomWordFromArray = easyWords[Math.floor(Math.random() * easyWords.length)];
        wordSplitToArray = randomWordFromArray.split('');
    } else {
        randomWordFromArray = hardWords[Math.floor(Math.random() * hardWords.length)];
        wordSplitToArray = randomWordFromArray.split('');
    }
};
function loser() {
